...
class ModelExtractor:
    """
    A utility class for extracting information about models and tensor
    operations from CSV files.
    """

    def __init__(self, models_path: str, tensors_path: str):
        """
        Initializes the ModelExtractor with paths
        to the model and tensor CSV files.
        ...
        - self.tensors_path (str): Stores the path to
            the tensor operations file.
        - self.model_dict (dict[str, list] or None): A dictionary representing
            model-related data (loaded from CSV).
    ...
    def load_model_dict(self) -> dict[str, list]:
        """
        Loads the model dictionary from the CSV file
        specified in `self.models_path`.

        Returns:
        - dict[str, list]: A dictionary where the keys
          are column names from the CSV
          and the values are lists of column data.

        Raises:
        - FileNotFoundError: If the CSV file at `self.models_path`
             cannot be found.
        ...
        if "method" not in df.columns or "library" not in df.columns:
            raise ValueError(
                "Expected columns 'method' and 'library'"
                f"not found in {self.models_path}"
            )
    ...
    def load_tensor_operations_dict(self) -> dict[str, list]:
        """
        Loads the tensor operations dictionary from the CSV
        file specified in `self.tensors_path`.
        ...
        - dict[str, list]: A dictionary where the keys
          are column names from the CSV
          and the values are lists of column data, filtered
          to include only operations
          with more than one tensor input.

        Raises:
        - FileNotFoundError: If the CSV file at `self.tensors_path`
          cannot be found.
        - ValueError: If the CSV does not contain the expected columns.
        """
    ...
    def load_model_methods(self) -> list[str]:
        ...
        Raises:
        - ValueError: If the model dictionary has not been
          loaded or does not contain the 'method' column.
        """
    ...
    def check_model_method(self, model: str, libraries: list[str]) -> bool:
        ...
        Returns:
        - bool: True if the model belongs to any of the specified libraries;
          False otherwise.
